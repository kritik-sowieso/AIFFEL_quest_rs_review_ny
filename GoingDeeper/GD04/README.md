...
- [x] **3. 에러가 난 부분을 디버깅하여 문제를 해결한 기록을 남겼거나 새로운 시도 또는 추가 실험을 수행해봤나요?**
    ...
    - 개선 아이디어: 현재 `PCK@0.05`는 256×256 crop에 대한 정규화 거리 기준으로 구현되어 있으므로, README나 결과 해석에 **공식 MPII PCKh가 아니라 프로젝트 내부 상대 비교 지표**라는 점을 한 줄 더 명시하면 해석이 더 안전할 것 같습니다. 또한 `joints_vis`를 이용해 보이지 않는 관절을 PCK 계산에서 제외하는 mask를 추가하면 실제 보이는 관절 기준 비교가 더 명확해질 것 같습니다.

- [ ] **4. 회고를 잘 작성했나요?**
    - 마지막에 Validation Loss, PCK, 정성적 결과, 한계까지 정리한 **결과 해석**은 매우 잘 작성되어 있습니다.
    - 다만 프로젝트를 수행하며 새롭게 이해한 점, 어려웠던 점, 다음에 다시 실험한다면 바꾸고 싶은 부분 등의 개인적인 **회고 문단**은 별도로 보이지 않았습니다.
...
- [ ] **5. 코드가 간결하고 효율적인가요?**
    ...
    - 다만 `crop_roi`, Gaussian heatmap 생성 로직이 standalone 함수와 `Preprocessor` 메서드에 각각 존재해 일부 코드 중복이 있습니다. 한 구현만 공통 함수로 재사용하면 유지보수가 더 쉬워질 것 같습니다.
    - `compute_pck()`도 batch 안에서 sample별 Python loop를 돌고 있어, 이후 데이터가 커질 경우 heatmap argmax와 distance 계산을 batch tensor 연산으로 벡터화하면 더 효율적으로 만들 수 있을 것 같습니다.
    - 그래서 구조화는 잘 되어 있지만, PRT의 “코드 중복 최소화와 효율” 기준에서는 개선 여지가 있다고 판단했습니다.
...
추가로 PCK 계산에서 `joints_vis` mask를 적용하고, 현재 threshold가 공식 MPII PCKh가 아닌 crop 기준 상대 지표임을 명시한다면 결과 해석이 더 단단해질 것 같습니다. 전체적으로 모델 구조와 추론 과정의 주석이 자세해서 코드를 따라가며 Pose Estimation 흐름을 복습하기 좋았습니다.
